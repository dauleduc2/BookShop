          ...
          <div class="space-y-20">
            <div>
              <table class="w-full mt-4 text-gray-500 sm:mt-6">
                ...
                <thead
                  class="text-sm text-left text-gray-500 sr-only sm:not-sr-only"
                >
                  <tr>
                    <th
                      scope="col"
                ...
                <tbody
                  class="text-sm border-b border-gray-200 divide-y divide-gray-200  sm:border-t"
                >
                  <c:forEach
                    var="order"
                  ...
                  >
                    <tr>
                      <td class="py-6 pr-8">
                        <div>
                          <a
                            href="<%=Router.PRODUCT_IN_ORDER_CONTROLLER%>?orderId=${order.getOrderId()}"
                            class="text-indigo-600"
                            >View
                            <span class="hidden lg:inline">#${pos.count}</span
                            ><span class="sr-only"
                              >, ${order.getCreatedDate()}</span
                            ></a
                          >
                        </div>
                      </td>
                      ...
                      <c:if
                        test="${(order.getStatus() == 0) || (order.getStatus() == 1)}"
                      >
                        <td class="hidden py-6 font-medium sm:table-cell">
                          <a
                            href="<%=Router.CANCEL_ORDER_CONTROLLER%>?orderId=${order.getOrderId()}"
                            class="
                              inline-flex
                              items-center
                              px-2.5
